...
const SurveyPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      try {
        const apiUrl = `${process.env.REACT_APP_API_URL}/api/survey/user`;
        console.log("📡 /api/survey/user 요청 URL:", apiUrl);
        const response = await axios.get(apiUrl, {
          headers: { Authorization: `Bearer ${token}` },
        });
        console.log("📩 /api/survey/user 응답:", response.data);
  ...
  useEffect(() => {
    ...
    const fetchSurvey = async () => {
      setIsLoading(true);
      const token = localStorage.getItem("token");
      const apiUrl = `${process.env.REACT_APP_FLASK_API_URL}/get_survey`;
